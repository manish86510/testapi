...
class PostLikesView(APIView):
    ...
    def delete(self, request):
        pk = request.POST.get('id')
        place = get_object_or_404(PostLikes.objects.all(), pk=pk)
        place.delete()
        return Response({"message": "Like with id {} has been deleted.".format(pk)}, status=204)
...
def increment_likes(sender, instance, **kwargs):
    # model = Post
    post_name = instance.post
    # print(Post.__dict__)
    print(Post.objects.all())
    if Post.about_post == post_name:
        print(Post)
    # print(kwargs)
    # print("sender = ", sender, "instance = ", instance.post)


# def decrement_likes():


post_save.connect(receiver=increment_likes, sender=PostLikes)
